import {
    arrayBufferUint8HexIndexOf,
    concatBuffer,
    id3FrameParser,
    toDataView
} from '$lib/share/Parser';
import type { id3FrameParserType } from '$lib/share/Parser';
...
const getAndParseBuffer = async (uri: string, index: number = 0, justConcat = false) => {
    ...
    return { uri, index, bufferList: concatBuffer(bufferList), frameInfoList };
};
