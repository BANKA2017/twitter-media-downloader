...
const arrayBufferUint8HexIndexOf = (
    content: Uint8Array,
    find: number[] = [],
    offset = 0
): number => {
    ...
    while (true) {
        ...
        if (firstWord === -1) {
            ...
            !find
                .slice(1)
                .some(
                    (findItem, index) => content.indexOf(findItem, offset) !== firstWord + index + 1
                )
        ) {
            return firstWord;
...
export type id3FrameParserType = [
    'PRIV' | 'TXXX' | 'TIT3' | unknown,
    number,
    ArrayBuffer,
    ArrayBuffer | string | object
];
